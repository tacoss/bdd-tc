...
function compileMacros(target, options, annotations) {
  ...
  ['before', 'after'].forEach(key => {
    if (data[key]) {
      ...
      if (typeof annotations[key][data[key]] !== 'function') {
        throw new Error(`Invalid method '${data[key]}', given ${annotations[key][data[key]]}`);
      }

      const hook = annotations[key][data[key]].toString().substr(data[key].length);
      const code = `async t => $${data[key]}.call(t, ${JSON.stringify(data)})`;

      push(target.usedFunctions, {
        id: `$${data[key]}`,
      ...
      if (options.feature) {
        options.feature[`${key}Each`] = code;
      }

      if (options.scenario) {
        options.scenario[key] = code;
      }
    }
  });
}
