...
const RE_THIS_VAR = /([=:?]\s*)\bthis\b/g;
const RE_THIS_CALL = /^(\s*)([\w\s(,.)]*)\bthis\b/gm;
...
function fixThis(code) {
  return code
    .replace(RE_THIS_VAR, '$1await this')
    .replace(RE_THIS_CALL, '$1await $2this');
}
    ...
    const fixtures = Object.keys(sandbox.steps).map(label => {
      if (typeof sandbox.steps[label] !== 'function') {
        if (typeof context[label] === 'object' && !Array.isArray(context[label])) {
          context[label] = Object.assign({}, context[label], sandbox.steps[label]);
        } else {
      ...
      const code = fixThis(sandbox.steps[label].toString()
        .replace(label, '')
...
function compileMacros(target, options, annotations) {
  ...
  const input = omit(data, ['before', 'after', 'tags', 'only', 'skip', 'url']);
  ...
  ['before', 'after'].forEach(key => {
    if (data[key]) {
      ...
      toArray(data[key]).forEach(value => {
        ...
        const hook = fixThis(annotations[key][value].toString().substr(value.length));
