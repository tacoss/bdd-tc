...
const RE_SYMBOLS = /[$`]/g;
const USE_SNAPSHOT = Math.random().toString(36).substr(2);

function safe(value) {
  return JSON.stringify(value.replace(/[/?<>\\:*|"]+/g, ''));
}

function encode(test, context) {
  const input = test.data.slice();

  return safe(test.step.replace(/\$(\w+)/g, (_, $1) => {
...
}) => `import { takeSnapshot as __snapshot${USE_SNAPSHOT} } from 'bdd-tc';
...
${scenario.tests.map(test => `  await $step${steps.indexOf(test.path)}[\`${escapeSymbols(test.step)}\`](${
  ...
  })(t);${scenario.options.snapshot ? `\n  await __snapshot${USE_SNAPSHOT}(t, ${encode(test, context)});` : ''}`).join('\n')}
})${
  scenario.after ? `\n  .after(async t => {
